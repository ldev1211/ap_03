...
import 'package:google_speech/generated/google/cloud/speech/v1/cloud_speech.pb.dart' hide RecognitionConfig;
...
class PlayMusicBloc extends Bloc<PlayMusicEvent, PlayMusicState> {
  ...
  final record = Record();
  final serviceAccount = ServiceAccount.fromFile(File('stt-flutter-6b7e79229bb6.json'));
  var speechToText;
  ...
  Future<List<int>> _getAudioContent(String name) async {
    final path = 'D:\\AndroidProject\\ap_03_LuongQuocDien\\record\\$name';
  ...
  PlayMusicBloc(PlayMusicState initial) : super(initial) {
    ...
    on<KaraokeChangeLyricEvent> ((event,emit) async {
      ...
      emit(KaraokeChangeLyricState(event.currDuration,(indexScroll==-1)?0:indexScroll));
    });
